...
def _media_odd(entradas, label, total_alvo=None):
    vals = [
...
def flat_para_linha(flat):
    ...
    return {
        ...
        "odds_ft_home_team_win": _media_odd(odds_1x2, "Home"),
        "odds_ft_draw": _media_odd(odds_1x2, "Draw"),
        "odds_ft_away_team_win": _media_odd(odds_1x2, "Away"),
        "odds_ft_over15": _media_odd(odds_gols, "Over", 1.5),
        "odds_ft_over25": _media_odd(odds_gols, "Over", 2.5),
        "odds_ft_over35": _media_odd(odds_gols, "Over", 3.5),
        "odds_ft_over45": _media_odd(odds_gols, "Over", 4.5),
        "odds_btts_yes": _media_odd(odds_btts, "Yes"),
        "odds_btts_no": _media_odd(odds_btts, "No"),
        "_fixture_id": flat["fixture_id"], "_referee_id": flat.get("referee_id"),
        "_odds_cartoes": flat.get("odds", {}).get("255", []),
    }


def carregar_liga_sportmonks(path):
    ...
    with open(path) as f:
        for l in f:
            linhas.append(flat_para_linha(json.loads(l)))
