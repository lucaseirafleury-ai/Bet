...
from sportmonks_client import LEAGUE_IDS, puxar_fixtures_finalizados, token
...
def atualizar_tudo():
    ...
    for liga, path in CAMINHO_HIST.items():
        ...
        totais[liga] = puxar_fixtures_finalizados(tok, LEAGUE_IDS[liga], path)
    return totais
...
if __name__ == "__main__":
    ...
    for liga, total in totais.items():
        print(f"{liga}: {total} fixtures salvos em {CAMINHO_HIST[liga]}", flush=True)
